...
- `GET` /users/{user_id}
    - Try it out -> Click to expand, enter a number for the `user_id` parameter (let's use `2`), and click Execute. The Response provides details about `Jim Jacobs`.
- `GET` /accounts
    - Try it out -> Click to expand, enter a date within the last 5 years (let's use 2024-12-03), a minimum balance (say, 5000), and status Open, then click Execute. The Response shows Accounts opened in the past 5 years with at least $5000.
